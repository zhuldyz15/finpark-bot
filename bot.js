...
const TRAIN_METHOD =
  "ДОПОЛНИТЕЛЬНЫЕ ПРАВИЛА (из обучения по продажам Finpark) — оценивай по ним ещё строже: " +
  "(1) Цена называется твёрдо, коротко и уверенно, сразу пауза-молчание; никаких оправданий, «воды» и «колоссального объёма работы». Обоснование цены = перечень конкретных инструментов/работ (ДДС, ОПиУ, 20 инструментов и т.п.), а не абстракции. " +
  "(2) Бюджет выясняется ОТКРЫТЫМ вопросом «Какой у вас бюджет?». Озвучить вилку цен вместо этого — ошибка: клиент цепляется за нижнюю границу и требует скидку. " +
  "(3) Отработка возражений строго по алгоритму: понять суть («Подумать над чем? Что мешает начать сейчас?») → собрать все («Только это вас останавливает?») → проверить истинность и присоединиться («Если я объясню X, вы готовы работать?») → аргументировать через выявленные боли и конкретный инструмент-решение → проверить, что зашло → призыв к действию. Нельзя перескакивать между возражениями — зацепиться за одно и раскрутить его. " +
  "(4) Сильнейший приём: на «нужен тайм-аут/подумаю» спрашивать «ЗАЧЕМ?» (подвергнуть скепсису), а не «над чем/какой?» — иначе продавец «покупает проблему клиента» и сдаётся. " +
  "(5) Вопросы вида «смог ли я донести ценность?» бесполезны (клиент ответит «да», чтобы отстали) — не считай это плюсом. " +
  "(6) Фиксация сделки: не уходить без оплаты/символической предоплаты (перевод 1–2 мин) с объяснением её смысла как подтверждения намерений; договориться о конкретном следующем шаге с датой/временем; дожим — по телефону, не сообщением. " +
  "(7) Продать нужно ТРИ вещи: продукт, компанию и СЕБЯ как эксперта. Если клиент идёт «изучать в YouTube» вместо вопроса партнёру — экспертность не продана. " +
  "(8) Признаки провала: неуверенность и лишняя болтовня, быстрая сдача после второго контраргумента, советы клиенту как/из какого кармана платить (запрещено на первой встрече), клиент ведёт встречу и фамильярничает без осадки, не пойман противоречие в словах клиента, клиента отпустили без отработки возражений. Повышение цен — не начинать с него, использовать только как рычаг при «дорого» (зафиксировать цену предоплатой до повышения). ";
...
async function evaluateText(transcript) {
  ...
  const rich =
    "Ты — РОП (руководитель отдела продаж) Finpark. Разбери ТЕКСТ ДИАГНОСТИКИ как наставник партнёра-франчайзи, строго по чек-листу. " + ROP_METHOD + TRAIN_METHOD + REC_NOTE +
